...
export const session01: Session = {
  ...
  theoryBlocks: [
    ...
    {
      heading: "평균의 함정: ‘평균 학습자’는 없다",
      imageLayout: "side",
      body: "왼쪽 그림은 널리 공유되는 교육 풍자화입니다. 시험관이 원숭이·펭귄·코끼리·어항 속 물고기·물개·개에게 말합니다. **“공정한 선발을 위해 모두 같은 시험을 봅니다. 저 나무에 오르세요.”** 하나의 잣대로 서로 다른 재능을 재는 순간, 시험은 공정해 보여도 결과는 공정하지 않습니다.\n\n**평균의 함정**(the trap of average)이란, 평균이 집단을 요약하는 통계값일 뿐인데 이를 ‘실재하는 표준 인간’처럼 여겨 설계의 기준으로 삼는 오류를 말합니다. 1950년대 미 공군은 조종사 4,000여 명의 신체 10개 항목을 재어 ‘평균 체형’에 맞춘 조종석을 만들었지만, 분석 결과 **10개 항목 모두 평균 범위에 드는 조종사는 단 한 명도 없었습니다**(길버트 대니얼스의 분석, 토드 로즈 『평균의 종말』). 평균에 맞춘 조종석은 사실상 **아무에게도 맞지 않는 조종석**이었고, 해결책은 평균이 아니라 **조절 가능한 좌석**이었습니다.\n\n**교육에서의 의의**\n\n- ‘평균 학습자’를 가정한 일제식 수업과 단일한 평가는, 평균 체형 조종석처럼 **누구에게도 정확히 맞지 않습니다.**\n- 공정함은 ‘모두에게 같은 것’을 주는 것이 아니라 ‘**각자에게 맞는 길**’을 여는 것입니다. 교육의 ‘조절 가능한 좌석’이 바로 UDL의 다양한 수단이고, 개인 맞춤형 학습(3차시)입니다.\n- 디지털 기술은 이 조절 가능한 학습 환경을 현실적으로 구현하는 수단입니다. 그래서 평균의 함정은 디지털 교육이 필요한 가장 근본적인 이유이기도 합니다.",
      images: [
        {
          src: "/assets/sessions/01/average-trap.jpg",
          alt: "교육 풍자화 ‘Our Education System’. 시험관이 원숭이, 펭귄, 코끼리, 어항 속 물고기, 물개, 개에게 ‘공정한 선발을 위해 모두 같은 시험을 봅니다. 저 나무에 오르세요’라고 말하는 그림",
          caption:
            "‘Our Education System’ — 널리 공유되는 교육 풍자화. 함께 실린 인용문은 아인슈타인의 말로 알려져 있으나 실제 출처는 불분명합니다.",
        },
      ],
    },
    {

...
export const session01: Session = {
  ...
  theoryBlocks: [
    ...
    {
      heading: "CAST와 UDL: 모두를 위한 학습 설계",
      body: "**CAST(Center for Applied Special Technology)**는 ‘학습에 한계가 없을 때까지(Until learning has no limits)’를 표방하는 미국의 비영리 교육 연구·개발 단체로, **UDL(보편적 학습 설계, Universal Design for Learning)** 개념을 만들고 발전시켜 온 곳입니다.\n\n**UDL**은 학습자의 차이를 예외가 아니라 **기본값**으로 보고, 처음부터 다양한 길을 설계하는 틀입니다. 그 목표는 목적의식적이고 성찰적이며, 자원 활용에 능하고 진정성이 있으며, 전략적이고 실행 중심적인 **학습자 주도성(learner agency)**을 기르는 것입니다. UDL은 세 가지 원리로 이루어집니다.\n\n- **참여(Engagement)**: 왜 배우는가 — 흥미·동기·자기조절\n- **표상(Representation)**: 무엇을 배우는가 — 정보를 여러 방식으로 제시\n- **행동과 표현(Action & Expression)**: 어떻게 표현하는가 — 다양한 수행·표현 경로\n\n표준 문서와 각 원리의 세부 지침은 CAST 공식 사이트 [udlguidelines.cast.org](https://udlguidelines.cast.org/) 에서 확인할 수 있습니다.",
    },
    {
      heading: "UDL 가이드라인 3.0 한눈에 보기",
      body: "CAST의 [UDL Guidelines 3.0](https://udlguidelines.cast.org/) 은 세 원리를 세로 열로, 세 층위를 가로 행으로 배치한 **3 × 3 그래픽 조직자**로 정리됩니다.\n\n- **열(원리)**: 참여 · 표상 · 행동과 표현\n- **행(층위)**: **접근(Access)** → **지원/구축(Support)** → **집행 기능(Executive Function)**\n\n각 칸에는 설계 옵션과 세부 체크포인트(예: 7.1 선택권과 자율성 최적화, 2.2 해독 지원, 6.4 진행 상황 모니터링 등)가 번호로 정리되어 있어, 수업을 점검할 때 체크리스트처럼 쓸 수 있습니다.\n\n아래 표는 CAST가 제공하는 **한국어판 그래픽 조직자**입니다. 이미지를 클릭하면 확대해서 볼 수 있고, 원본은 [한국어 PDF](https://udlguidelines.cast.org/static/udlg3-graphicorganizer-digital-numbers-a11y-korean.pdf) 에서 내려받을 수 있습니다.",
      images: [
        {
          src: "/assets/sessions/01/udl-guidelines-ko.png",
          alt: "UDL 보편적 학습 설계 가이드라인 3.0 한국어 그래픽 조직자. 참여·표상·행동과 표현 세 원리를 열로, 접근·지원·집행 기능 세 층위를 행으로 배치한 3x3 표와 각 칸의 설계 옵션·체크포인트",
          caption:
            "UDL Guidelines 3.0 그래픽 조직자(한국어판). 클릭하면 확대됩니다. 출처: CAST (2024). Universal Design for Learning Guidelines version 3.0 [graphic organizer]. udlguidelines.cast.org © CAST, Inc.",
        },
      ],
    },
    {
      heading: "UDL과 디지털 교육의 관계",
      body: "디지털 교육과 UDL은 서로를 강하게 지지합니다. UDL이 말하는 ‘다양한 수단’은 아날로그 환경에서는 준비 부담이 크지만, **디지털 기술은 이를 실질적으로 가능하게** 합니다.\n\n- **표상의 다양화**: 음성 읽기(TTS), 자막, 번역, 글자 크기·색 대비 조절로 같은 내용을 여러 방식으로 제시\n- **행동과 표현의 다양화**: 글·음성·영상·그림 등 학습자가 선택한 방식으로 산출물 제작·제출\n- **참여의 최적화**: 선택권 제공, 즉각적 피드백, 진행 상황 시각화로 동기와 자기조절 지원\n\n다만 순서가 중요합니다. **기술이 먼저가 아니라 UDL 원리가 먼저**입니다. 도구는 ‘참여·표상·행동과 표현’이라는 목적에 맞을 때 의미가 있으며, 이는 앞서 본 **Alignment**(목표–활동–평가–도구 정렬)와 **ISTE**의 관점(학습 경험의 질)과 그대로 이어집니다. 또한 접근성, 개인정보, 디지털 윤리는 이 모든 설계의 전제입니다.",
    },
  ],
